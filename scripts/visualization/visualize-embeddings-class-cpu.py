...
Install:
    pip install umap-learn datashader scikit-learn
"""
...
try:
    ...
    raise ImportError(
        ...
        "Install: pip install umap-learn datashader scikit-learn adjustText scipy"
    )
...
EMBEDDINGS_DIR = "output/B4/embeddings_partitioned"
PREDICTIONS_DIR = "output/B4/predictions_partitioned"         
OUTPUT_DIR = "output/B4/umap_visualization"
...
def _matches_audio(file_field: str, file_path_field: str, audio_stem: str) -> bool:
    ...
    Logic mirrors visualize.py: prefer exact stem match on file_path, then fall back
    to prefix match on `file` (which may include chunk suffixes like _12).
    """
...
def filter_for_audio(df: pd.DataFrame, audio_path: Path) -> pd.DataFrame:
    ...
    if filtered.empty:
        ...
        # Sort by chunk_idx to make expected 5s segments evident
...
def create_class_plot(
    ...
    output_dir: str,
    highlight_points: Optional[List[Tuple[float, float, str]]] = None,
    ...
    plot_range = max(extent[1] - extent[0], extent[3] - extent[2])
    ...
    # Highlight specific embeddings if provided (e.g., chunks from --audio-file)
    if highlight_points:
        center = np.array([(extent[0] + extent[1]) / 2, (extent[2] + extent[3]) / 2])
        for hx, hy, label in highlight_points:
            vec = np.array([hx, hy]) - center
            norm = np.linalg.norm(vec)
            if norm < 1e-6:
                vec = np.array([1.0, 0.0])
                norm = 1.0
            direction = vec / norm
            offset = 0.03 * plot_range
            text_xy = np.array([hx, hy]) + direction * offset

            ax.scatter(hx, hy, s=35, color="#ffd166", edgecolors="black", linewidths=0.8, zorder=110)
            ax.plot([hx, text_xy[0]], [hy, text_xy[1]], color="#ffd166", lw=1.0, alpha=0.9, zorder=109)
            ax.text(
                text_xy[0],
                text_xy[1],
                label,
    ...
    plt.savefig(
        f"{output_dir}/class_embeddings_{audio_path.name}.png",
        dpi=300,
        bbox_inches="tight",
        pad_inches=0.1,
        facecolor="#1a1d21",
    ...
    plt.savefig(
        f"{output_dir}/class_embeddings_{audio_path.name}.svg",
        bbox_inches="tight",
        pad_inches=0.1,
        facecolor="#1a1d21",
        edgecolor="none",
...
def parse_args():
    parser = argparse.ArgumentParser(description="UMAP class visualization (CPU)")
    parser.add_argument(
        "--embeddings-dir",
    ...
    parser.add_argument(
        "--audio-file",
        type=str,
        help="Optional audio file path to visualize only that file's embeddings/predictions",
    )
    parser.add_argument(
        "--max-files",
        type=int,
        default=MAX_FILES,
        help="Limit number of parquet files loaded (debugging)",
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=N_WORKERS,
        help="Worker threads for parquet loading",
    )
    parser.add_argument(
        "--top-classes",
        type=int,
        default=N_TOP_CLASSES,
        help="Number of top classes to label",
    )
...
def main():
    ...
    if audio_path is not None:
        audio_subset = filter_for_audio(df, audio_path)
        console.print(
            f"  Matched embeddings for audio '{audio_path.name}': {len(audio_subset):,} rows"
        )
    ...
    if audio_path is not None:
        audio_pred_subset = filter_for_audio(df_pred, audio_path)
        console.print(
            f"  Matched predictions for audio '{audio_path.name}': {len(audio_pred_subset):,} rows"
        )
    ...
    if audio_path is not None and "chunk_idx" in df.columns:
        # Find merged rows corresponding to the audio file (on merged df to preserve order)
        audio_rows = filter_for_audio(df, audio_path)
        unique_chunks = sorted(audio_rows["chunk_idx"].dropna().unique())
        console.print(
            f"  Chunks found for audio: {len(unique_chunks)} (indices: {unique_chunks[:24]}{'...' if len(unique_chunks) > 24 else ''})"
        )

        # Build highlight list using start_time if available, else derive from chunk_idx
        highlight_points = []
        for row_idx, row in audio_rows.iterrows():
            cx = row.get("chunk_idx")
            start_time = row.get("start_time")
            label_time = start_time if pd.notna(start_time) else (cx * 5 if pd.notna(cx) else None)
            if label_time is None:
                label = ""
            else:
                try:
                    label = f"{int(label_time)}s"
                except Exception:
                    label = f"{label_time}s"
            highlight_points.append((int(row_idx), label))
    ...
    console.print(Panel.fit("→ Running CPU UMAP...", border_style="cyan"))
    umap = UMAP(
        n_components=2,
        metric="euclidean",
    ...
    # Build highlight coordinates if requested
    highlight_coords = None
    if highlight_points is not None:
        # highlight_points currently stores (row_index, label)
        highlight_coords = []
        for row_idx, label in highlight_points:
            if 0 <= row_idx < len(embeddings_2d):
                x, y = embeddings_2d[row_idx]
                highlight_coords.append((float(x), float(y), label))
    ...
    create_class_plot(
        ...
        top_classes,
        output_dir,
        highlight_points=highlight_coords,
